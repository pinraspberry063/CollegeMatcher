---
title: RoomateMatcher
---
<SwmSnippet path="/app/RoomateMatcher.jsx" line="17">

---

Instantiation of options for responses and their corresponding variables by setting commands to store the string responses a
...
<SwmSnippet path="/app/RoomateMatcher.jsx" line="154">

---

A function for updating the choice variables to the chosen response by the user. The user is then found by using their uid and the data is saved inside of their User data page in firebase.
...
<SwmSnippet path="/app/RoomateMatcher.jsx" line="220">

---

Each of the three pages are rendered with text headers describing question and a dropdown mechanic for choosing a preset response. Each page is rendered seperatly and a series of buttons are rendered at the bottom of the page for navigating between them. A submit button is rendered on page 3   that will run the handleSubmit function in order to save the choices. The saved data is put through a matchRoomates in order to calculate the top 5 roomates compatible and the navigate to RoomateResults using that data to display it.
