...
const matchColleges = async studentPreferences => {
  ...
  const scores = colleges.map(college => {
    ...
    const [minTuition, maxTuition] = tuitionRanges[
      studentPreferences.tuition_cost
    ] || [0, Infinity];
    const isInMainRange = tuition >= minTuition && tuition <= maxTuition;
    const isInNextClosestRange =
      selectedTuitionRangeIndex > 0 &&
      selectedTuitionRangeIndex < rangeOrder.length - 1 &&
      (tuitionRanges[rangeOrder[selectedTuitionRangeIndex - 1]].includes(
        tuition,
      ) ||
        tuitionRanges[rangeOrder[selectedTuitionRangeIndex + 1]].includes(
          tuition,
    ...
    if (
      studentPreferences.major === 'Computer Science' ||
      studentPreferences.major === 'Computer Information Systems'
    ) {
      const majorValue = parseFloat(
        college.percent_Computer_and_ComputerInfoScienciesDegrees,
      );
      if (majorValue > 4) {
        score += 20;
    ...
    const [minDistance, maxDistance] = distanceRanges[
      studentPreferences.distance_from_college
    ] || [0, Infinity];
